...
urlpatterns = [
    ...
    # board 에선 경로 바껴서 바꿔야함
    path("login/", auth_views.LoginView.as_view(template_name="common/login.html"), name="login"),
    path("logout/", auth_views.LogoutView.as_view(), name="logout"),
    # 기존 비밀번호 변경 후 세션 값 다시 담아줌
    # common:password_reset_done 내 거랑 연결
    path("password_change/", auth_views.PasswordChangeView.as_view(template_name="common/password_change.html",
                                                                   success_url=reverse_lazy("common:password_reset_done")), name="password_change"),
    # 비밀번호 초기화
    # 장고 reset 가는 거
    # path("password_reset/", auth_views.PasswordResetView.as_view(), name="password_reset"),
    # 내 reset html 가기
    path("password_reset/", auth_views.PasswordResetView.as_view(template_name="common/password_reset.html",
    ...
    path("reset/<uidb64>/<token>/", auth_views.PasswordResetConfirmView.as_view(template_name="common/password_reset_confirm.html",
